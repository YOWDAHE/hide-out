...
export default function MessagesList({
	...
	// Sync local state whenever server sends new messages prop
	useEffect(() => {
	...
	useEffect(() => {
		...
		const handler = (payload: { conversationId: string; message: Message }) => {
		...
		<div className="flex-1 overflow-y-auto px-4 pb-20 flex items-end w-full">
			<div className="max-w-3xl mx-auto w-full">
				{localMessages.map((msg, idx) => {
					...
					const nextMsg = localMessages[idx + 1] ?? null;
					const showTimestamp =
						!nextMsg ||
